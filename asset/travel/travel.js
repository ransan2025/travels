...
let deleting = false;
function typeLoop() {
    const word = words[txtIndex];
    if (!deleting) {
        typingText.innerHTML = word.substring(0, charIndex);
        charIndex++;
...
setInterval(() => {
    spot++;
    if (spot >= spotItems.length) spot = 0;
    spotlightBox.innerHTML = spotItems[spot];
}, 2500);
...
setInterval(() => {
    i = (i + 1) % badgeTexts.length;
    badgeEl.style.opacity = 0;
    setTimeout(() => {
        badgeEl.textContent = badgeTexts[i];
...
(function () {
    ...
    function rotateDestination() {
        ...
        rotateEl.innerText = destinations[destIndex];
    }
    setInterval(rotateDestination, 2500);

    /* -----------------------------
       LIVE SEAT AVAILABILITY
    ------------------------------ */
    function updateSeats() {
        const seatEl = document.getElementById("seatLive");
        if (!seatEl) return;
        const seats = Math.floor(Math.random() * 35) + 8;
        seatEl.innerText = `Seat Availability: ${seats} seats left today`;
    }
    setInterval(updateSeats, 3200);
    updateSeats();

    /* -----------------------------
       OPTIONAL: LOWEST FARE TICKER (future-ready)
    ------------------------------ */
    const fares = ["₹4,999", "₹5,499", "₹3,999", "₹6,299", "₹4,199"];
    let fareIndex = 0;
    const fareEl = document.querySelector(".chip.price b");
    function updateFare() {
        if (!fareEl) return;
        fareIndex = (fareIndex + 1) % fares.length;
        fareEl.innerText = fares[fareIndex];
    }
    setInterval(updateFare, 4000);
})();
...
const SCRIPT_URL =
    "https://script.google.com/macros/s/AKfycbwIQx-mRIvXV0yaqeg0ejY1K6lwM-cPCi9PShCjh8GwJZjIFLLqdHaoVf-mVRX7JFX4/exec";
...
const response =
    await fetch(
        ...
        "https://script.google.com/macros/s/AKfycbxzjG7PzGYXumIEVVEDkM3WRqeH6dZjUsY7uQdk0n9z0nygYDgcPfyopGlx6o2PZKTo/exec?action=getCarRentals"
