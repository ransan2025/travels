...
/* =========================================================
   RANSAN DASHBOARD
   PAYMENT VISIBILITY HELPERS
   ---------------------------------------------------------
   Read-only enhancement.
   Does NOT update the backend.
   Does NOT change existing booking data.
   ========================================================= */

function getDashboardPaymentNumber(value) {
...
/* =========================================================
   FIRST EXISTING PAYMENT VALUE
   ========================================================= */

function getDashboardPaymentField(
    row,
    keys
) {
...
/* =========================================================
   TOTAL AMOUNT
   ---------------------------------------------------------
   Uses your existing naming variations.
   No field is written or modified.
   ========================================================= */
...
function getDashboardPaymentTotal(
    row
) {
    ...
    /*
     * Fall back to your existing Dashboard helper.
     */
    if (
        typeof getBookingAmount ===
        "function"
    ) {
...
function getDashboardPaymentPaid(
    row
) {
...
/* =========================================================
   BALANCE AMOUNT
   ---------------------------------------------------------
   If Balance Amount already exists, use it.

   Otherwise:
   Balance = Total - Paid

   Calculation is display-only.
   ========================================================= */
...
function getDashboardPaymentBalance(
    row,
    total,
    paid
) {
    ...
    /*
     * If the Sheet already contains a balance,
     * use that value.
     */
    if (
        number !== null
    ...
    /*
     * Otherwise calculate the display balance.
     */
    if (
        total > 0
    ) {
...
/* =========================================================
   PAYMENT STATUS
   ---------------------------------------------------------
   Existing Payment Status always wins.

   Only if it does not exist do we infer a display status.
   ========================================================= */
...
function getDashboardPaymentStatus(
    row,
    total,
    paid,
    balance
    ...
    /*
     * If the Sheet already contains a Payment Status,
     * always respect that value.
     */
    if (explicit !== null) {
    ...
    /*
     * No financial value yet.
     *
     * Example:
     * New enquiry where the final fare
    ...
    /*
     * Amount exists but no payment received.
     */
    if (
        total > 0 &&
        paid <= 0
...
function getDashboardPaymentStatusClass(
    status
) {
...
function formatDashboardPaymentAmount(
    value
) {
...
/* =========================================================
   SHOULD PAYMENT PANEL BE DISPLAYED?
   ---------------------------------------------------------
   V2:
   Show the payment section for every booking / enquiry.

   This is display-only.
   It does NOT create or save payment information.
   ========================================================= */

function shouldShowDashboardPayment(
    row,
    total,
    paid,
    balance,
    ...
    /*
     * Payment visibility should remain available
     * even when the booking is still:
     *
     * New
     * Pending
     * Follow Up
     ...
     * Completed
     *
     * This makes finance information consistently
     * visible throughout the booking lifecycle.
     */
...
/* =========================================================
   REGISTER BOOKING FOR PAYMENT MODAL
   ========================================================= */

function registerDashboardPaymentRow(
    row
) {
...
/* =========================================================
   BUILD PAYMENT VISIBILITY PANEL
   ========================================================= */

function renderDashboardPaymentVisibility(
    row
) {
    ...
    const statusClass =
        getDashboardPaymentStatusClass(
            paymentStatus
        );
    ...
    return `
        ...
        <div
            class="dashPaymentVisibility
                   dashPaymentVisibility-${statusClass}"
        >

            <!-- HEADER -->

            <div class="dashPaymentHeader">

    <div class="dashPaymentHeading">

        <span class="dashPaymentHeadingIcon">
            ₹
        </span>
    ...
    <div class="dashPaymentHeaderActions">


        <span
            class="
                dashPaymentStatus
                dashPaymentStatus-${statusClass}
            "
        >
            ${escapeDashboardPaymentHtml(
        paymentStatus
    )}
        </span>
        ...
        total > 0 &&
            ...
            ? `
                <span
                    class="dashPaymentCompletedBadge"
            ...
            /*
             * NOT FULLY PAID / FARE NOT SET
             *
             * Always display Add Payment.
             * Validation remains inside the modal
             * and server.
             */
            `
                <button
                ...
                paymentKey
                    ...
                    title="
                        ${total <= 0
                ? "Final Fare must be entered before recording payment"
                : "Record payment"
            }
                    "
                >
            ...
            <div class="dashPaymentGrid">


                <div class="dashPaymentMetric total">
            ...
            total
                ...
                <div class="dashPaymentMetric paid">
            ...
            paid
                ...
                <div class="dashPaymentMetric balance">
            ...
            <div class="dashPaymentProgressRow">

                <div class="dashPaymentProgress">
                ...
                <strong class="dashPaymentPercentage">
                    ${percentage}%
                </strong>
...
<div class="dashPaymentProgressText">

    ${total <= 0

            ? "Final Fare not set yet"
            ...
            : percentage >= 100
                ...
                : percentage > 0
                    ...
                    ? `${percentage}% received • ${formatDashboardPaymentAmount(balance)} pending`

                    : `${formatDashboardPaymentAmount(balance)} pending`
        }
...
function escapeDashboardPaymentHtml(
    value
) {
...
${renderDashboardPaymentVisibility(row)}
